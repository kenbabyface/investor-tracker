...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-50">
        <div class="flex h-screen overflow-hidden">
                ...
                <div class="flex flex-col h-full">
                    ...
                    <div class="flex items-center justify-between p-6 border-b border-blue-700/50 bg-gradient-to-r from-blue-900/50 to-transparent">
                        <div class="flex items-center space-x-3">
                            <div class="icon-container bg-white/20 p-2.5 rounded-xl shadow-lg">
                                <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                            </div>
                            <div>
                                <h1 class="text-lg font-bold tracking-tight">Investor Tracker</h1>
                                <p class="text-xs text-blue-200 font-medium tracking-wide uppercase">Portfolio Management</p>
                            </div>
                        </div>
                        <button id="closeSidebar" class="lg:hidden text-white/80 hover:text-white p-1 rounded-lg hover:bg-white/10 transition-colors">
                    ...
                    <nav class="flex-1 overflow-y-auto custom-scrollbar py-5 px-3 space-y-1">
                        ...
                        ══════════════════════════════════ -->
                        @php
                            $investOpen = request()->routeIs('investments.*') || request()->routeIs('payments.schedule');
                        @endphp

                        <div>
                            <button type="button"
                                    id="btn-investments"
                                    onclick="toggleNav('investments')"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <svg id="chevron-investments"
                                         class="nav-chevron w-4 h-4 text-blue-300 {{ $investOpen ? 'rotated' : '' }}"
                            ...
                            <div id="dropdown-investments"
                                 class="nav-dropdown {{ $investOpen ? 'open' : '' }} ml-4 mt-1">
                                <div class="pl-3 border-l border-blue-600/40 space-y-0.5 pb-1">

                                    <a href="javascript:void(0)"
                                       onclick="openPasswordModal('{{ route('investors.create') }}')"
                        ...
                        ══════════════════════════════════ -->
                        @php
                            $fishOpen = request()->routeIs('ponds.*') || request()->routeIs('feed-logs.*') || request()->routeIs('feed-sizes.*');
                        @endphp

                        <div>
                            <button type="button"
                                    id="btn-fish"
                                    onclick="toggleNav('fish')"
                                    class="nav-group-btn {{ $fishOpen ? 'group-open' : '' }} w-full flex items-center justify-between px-4 py-3 rounded-xl">
                                <div class="flex items-center space-x-3">
                                    <div class="icon-container p-1.5 rounded-lg bg-white/10">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M21 12c-2-4-6-7-9-7S5 8 3 12c2 4 6 7 9 7s7-3 9-7z"/>
                                            <circle cx="15.5" cy="11" r="1" fill="currentColor" stroke="none"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M3 12C2 10.5 1.5 8.5 2 7M3 12c-1 1.5-1.5 3.5-1 5"/>
                                        </svg>
                                    </div>
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="group-dot w-1.5 h-1.5 bg-blue-300 rounded-full"></span>
                                    <svg id="chevron-fish"
                                         class="nav-chevron w-4 h-4 text-blue-300 {{ $fishOpen ? 'rotated' : '' }}"
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                    </svg>
                                </div>
                            ...
                            <div id="dropdown-fish"
                                 class="nav-dropdown {{ $fishOpen ? 'open' : '' }} ml-4 mt-1">
                                <div class="pl-3 border-l border-blue-600/40 space-y-0.5 pb-1">
                        ...
                        <div class="mt-5 pt-5 border-t border-blue-700/50">
                            ...
                            <form method="POST" action="{{ route('logout') }}" class="mt-1">
                                ...
                                <button type="submit" class="sidebar-link w-full flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-red-500/20 text-left group">
                                    <div class="icon-container p-1.5 rounded-lg bg-white/10 group-hover:bg-red-500/20">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- ═══════════════════ MAIN CONTENT ═══════════════════ -->
            <div class="flex-1 flex flex-col lg:ml-64 min-h-screen bg-gray-50">
                <header class="bg-white header-shadow border-b border-gray-200 sticky top-0 z-40">
                    <div class="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-4">
                        <div class="flex items-center gap-4 flex-1">
                            <button id="openSidebar" class="lg:hidden text-gray-600 hover:text-blue-600 p-2 rounded-lg hover:bg-gray-100 transition-colors focus:outline-none">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
